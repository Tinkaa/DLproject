...
def init_params(options):
    # Specify CNN to use: either 'vgg' or 'resnet50'
    ...
    try:
        ...
        print(
            'Could not load pre-trained model weights. Weights can be found in the keras application folder https://github.com/fchollet/keras/tree/master/keras/applications')
    ...
    if options.loss_func == 'frcnn':
        ...
        model_rpn.compile(optimizer=optimizer,
                          loss=[focal_loss.rpn_focal_loss_cls(num_anchors), focal_loss.rpn_smooth_l1_regr(num_anchors)])
    else:
...
def train(data_gen_train, model_rpn, model_classifier, model_all, num_epochs, epoch_length):
    iter_num = 0
    ...
    for epoch_num in range(num_epochs):
        ...
        while True:
            try:
                ...
                R = roi_helpers.rpn_to_roi(P_rpn[0], P_rpn[1], C, use_regr=True, overlap_thresh=0.7,
                                           max_boxes=300)
...
if __name__ == "__main__":
    ...
    data_gen_train, data_gen_test, model_rpn, model_classifier, model_all = init_params(options)

    epoch_length = 1000
    num_epochs = int(options.num_epochs)

    train(data_gen_train, model_rpn, model_classifier, model_all, num_epochs, epoch_length)
