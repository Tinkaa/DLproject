...
class_names=list(class_mapping)[:-1]
for i in list(class_mapping)[:-1]:
    open('results/VOC2007/Main/comp3_det_test_' + i + '.txt', 'w').close()
...
all_imgs, classes_count, _=get_img_list(img_path,class_names)
print('Predicting')
...
#for idx, img_name in enumerate(sorted(os.listdir(img_path))):
    ...
    for key in bboxes:
        ...
        result_file = 'results/VOC2007/Main/comp3_det_test_' + key + '.txt'
        new_boxes, new_probs = roi_helpers.non_max_suppression_fast(bbox, np.array(probs[key]), overlap_thresh=0.5)
        img_name_noext = os.path.splitext(img_name.rsplit('/', 1)[-1])[0]
        with open(result_file, 'a') as f:
            for i, j in zip(new_boxes, new_probs):
                f.write("%s %f %f %f %f %f\n" % (img_name_noext, j, i[0], i[1], i[2], i[3]))
...
# #cv2.imshow('img', img)
# #cv2.waitKey(0)
# cv2.imwrite('./results_imgs/giraffe/{}.png'.format(idx),img)
